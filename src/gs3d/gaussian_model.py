...
class GaussianModel:
    ...
    def __init__(self, sh_degree, dim_embed=0, embed=None, active_uncertainty=True):
    ...
    def set_embed(self, num_cameras):
        if self.dim_embed > 0:
            if self.embed == 'gaussianimage':
                ...
                # self.embedding.weight.data.view(num_cameras, self.dim_embed, 8)[:, :, 2:5] = torch.rand(
                #     (num_cameras, self.dim_embed, 3))  # rgb
                ...
                self.gaussianimage = GaussianImage(init_num_points=self.dim_embed, active_uncertainty=self.active_uncertainty).cuda()
    ...
    def save_mlp_checkpoints(self, path):  # split or unite
        ...
        if self.dim_embed > 0:
            self.embedding.eval()
            emd = torch.jit.trace(self.embedding, (torch.zeros((1,), dtype=torch.long).cuda()))
            emd.save(os.path.join(path, 'embedding.pt'))
            self.embedding.train()

    def load_mlp_checkpoints(self, path):
        if self.dim_embed > 0:
            self.embedding = torch.jit.load(os.path.join(path, 'embedding.pt')).cuda()
