...
# from pycolmap.pycolmap import SceneManager
    ...
    def __init__(
        ...
        for k in imdata:
            ...
            if downsample:
                ...
                # resize_im = media.resize_image(im_rgb, imsize_dict[camera_id])
                resize_im = media.resize_image(im_rgb, (imsize_dict[camera_id][1], imsize_dict[camera_id][0]))
        ...
        # image_to_colmap = dict(zip(image_files, colmap_files))
        # import pdb; pdb.set_trace()
        # image_files 180
        # colmap_files 179
        # image_names 179
        # import pdb; pdb.set_trace()
        # 找出colmap_files和image_files中不同的文件
        # diff_files = set(colmap_files) - set(image_files)
        # diff_files1 = set(colmap_files) - set(colmap_files)
        # len(colmap_to_image.keys())
        # print(diff_files)
        image_paths = [os.path.join(image_dir, colmap_to_image[f]) for f in image_names]
        # '2clutter030.JPG' in image_files # False
        # '2clutter030.JPG' in image_names # False
        # '2clutter030.JPG' in colmap_files # True
...
class Dataset:
    ...
    def __getitem__(self, item: int) -> Dict[str, Any]:
        # print(self.indices.__len__(), item)
...
class ClutterDataset(Dataset):
    ...
    def __init__(
        ...
        if train_keyword == "":
            if split == "train":
                if os.path.exists(os.path.join(self.parser.data_dir, "train_list.txt")):
                    print("Found train_list.txt, using it to split train and test set.")
                    with open(os.path.join(self.parser.data_dir, "train_list.txt")) as f:
                                    train_list = f.read().splitlines()
                    self.indices = [i for i, name in enumerate(self.parser.image_names) if name in train_list]
                    # import pdb; pdb.set_trace()
                else:
                    self.indices = indices[indices % self.parser.test_every != 0]
            else:
                if os.path.exists(os.path.join(self.parser.data_dir, "test_list.txt")):
                    print("Found test_list.txt, using it to split train and test set.")
                    with open(os.path.join(self.parser.data_dir, "test_list.txt")) as f:
                                    test_list = f.read().splitlines()
                    self.indices = [i for i, name in enumerate(self.parser.image_names) if name in test_list]
                    # import pdb; pdb.set_trace()
                    print(self.indices)
                else:
                    self.indices = indices[indices % self.parser.test_every == 0]
